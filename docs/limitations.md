...
**Scope of the provider-enforced condition (dogfood finding DF-F2):** the CAS covers the resources the effect **writes** — the executor enforces the authorized expected state for exactly those refs and fails closed when it cannot. A dependency the effect only *reads* that drifts between authorization and execution is **not** re-verified by the conditional path (the legacy fingerprint re-check covered it best-effort). Concretely: action parameters derived from read-only dependencies can be applied from values that were current at authorization but changed before execution. If read-only drift matters for an action, restructure the intent so the drifted resource is the conditioned (written) one, verify it with a precondition the provider can vouch for, or re-observe and resubmit between authorization-sensitive steps. Per-resource CAS is the providers' contract; multi-resource atomicity does not exist on any supported provider and is not claimed.
