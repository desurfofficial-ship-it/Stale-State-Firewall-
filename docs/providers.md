...
|---|---|---|---|---|---|---|
| In-memory | FULL | **VERIFIED** (automated, every CI run) | contract suite + continuous harness | 2026-09-06 | `test/contract/providers.test.ts`; harness scenarios 01–11; sustained-dogfood incident exercise (SQLite-persisted CAS-window refusal + replay refusal, CLI-reconstructable); FL-9 regression pins (mutate() always bumps the version → stale CAS refuses; put() version-preserving on existing resources is documented re-seeding semantics) | reference provider; single-process event-loop atomicity; `put()` on an EXISTING resource deliberately preserves its version — documented re-seeding convenience, never for simulating external actors (use `mutate()`; see INTERNAL_DOGFOOD_LOG FL-9, resolved 2026-09-06) |
| GitHub `file` | FULL | **VERIFIED** (live API, dedicated sandbox repo only) | dogfood harness scenarios 12 + 13 (live runs) | 2026-09-06 | CAS satisfied → mutation applied; blob-sha moved in CAS window → GitHub refused the stale write (409-class); two independent agents racing → exactly one lands (GitHub decides); no credentials in audit records. Deep campaign: dogfood S13. Continuous-dogfood adoption workflow runs the full agent loop (interference → stale refusal → autonomous recovery) against the same sandbox. **First on-Actions live run `34020936423` (success, 14/14); fail-closed polarity proven by run `34020483097` (no secret → guard failure, live step skipped). Sustained-dogfood 2026-09-06: live 14/14 re-run with the current credential + new WF-2 dependency-update workflow first-run PASS (dependabot-style lockfile drift DENY at validation; DF-F2 CAS-window boundary re-confirmed on the dependency-update class)** | GitHub other resources: UNSUPPORTED (no expected-revision parameter); 409 responses carry no current sha (`observed_version: null` — see INTERNAL_DOGFOOD_LOG FL-4) |
...
Version semantics of `put()` vs `mutate()` (deliberate, FL-9): `put()` seeds; on an EXISTING resource without an explicit `version` argument it keeps the current version (a re-seeding convenience — the content replace is invisible to CAS comparisons held at the old version). `mutate()` always advances the version: that is the primitive for simulating a concurrent external actor. Pinned by regression tests in `test/contract/providers.test.ts`.
