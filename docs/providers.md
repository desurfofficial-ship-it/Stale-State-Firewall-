...
|---|---|---|---|---|---|---|
| In-memory | FULL | **VERIFIED** (automated, every CI run) | contract suite + continuous harness | 2026-09-06 | `test/contract/providers.test.ts`; harness scenarios 01–11; sustained-dogfood incident exercise (SQLite-persisted CAS-window refusal + replay refusal, CLI-reconstructable) | reference provider; single-process event-loop atomicity; `put()` on an EXISTING resource keeps its version (CAS-invisible replace — use `mutate()` for external actors; see INTERNAL_DOGFOOD_LOG FL-9) |
