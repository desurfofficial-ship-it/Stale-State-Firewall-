...
- **Enforcement scope (dogfood finding DF-F2):** the provider-enforced condition covers the resources the effect **writes** — the executor enforces the authorized expected state for exactly those refs. A read-only dependency that drifts between authorization and execution is NOT re-verified by the conditional path (the legacy fingerprint re-check covered it best-effort). Parameters derived from read-only dependencies can therefore be applied from authorization-time values. See docs/limitations.md for the operator guidance.
